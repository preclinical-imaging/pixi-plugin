    ...
    public List<String> call() throws ClientException, ServerException {
        ...
        if (format == Format.ZIP) {
            try (final ZipInputStream zin = new ZipInputStream(in)) {
                log.info("Zip file received by Inveon importer.");
...
//                cleanupOnImportFailure();
    ...
    private void processSessionMap(String projectId) throws ServerException, IOException {
        ...
        while (it.hasNext()) {
            ...
            log.info("Session Label {} with {} files", inveonSessionFiles.getSessionLabel(), inveonSessionFiles.getInveonImageMap().size());
        }
        ...
        while (inveonSessionFilesIterator.hasNext()) {
            ...
            log.info("Session label: {}", inveonSessionFiles.getSessionLabel());
            log.info("Session Files Count {}", inveonSessionFiles.getInveonImageMap().size());
            ...
            while (keyIterator.hasNext()) {
                ...
                log.info("Key: {}, value {} {} {}", key, map.get(key).getModality(), map.get(key).getHeaderFileName(), map.get(key).getPixelFileName());
...
/*    protected ZipEntry importDirectory(final String projectId, ZipEntry ze, ZipInputStream zin) throws IOException, ServerException, ClientException {
    ...
    protected ZipEntry stageInveonFile(final String projectId, ZipEntry ze, ZipInputStream zin) throws IOException, ServerException, ClientException {
        ...
        log.info("Importing file: {}", pathName);
        ...
        // TODO This is all based on WUSTL PCIF naming conventions and needs to be generalized.
        String imageName = FilenameUtils.getBaseName(name);
        String sessionLabel = extractSessionLabel(fileName);
        String scanLabel    = extractScanLabel(fileName);

        log.info("Session Label: {}, Scan Label: {}", sessionLabel, scanLabel);
        ...
        if (keepFile) {
            ...
            if (inveonImageRepresentation == null) {
                ...
                //inveonImageRepresentation.setModality(modality);
            ...
            log.info("About to place inveonSessionFiles in the Session Files Map, label is {} {}", sessionLabel, inveonSessionFiles.getSessionLabel());
    ...
    private void processCTSession(InveonSessionFiles inveonSessionFiles, String projectId) throws ServerException, IOException {
        // TODO Add code for a session with only CT files
        log.error("Should not be in processCTSession");
    }
    ...
    private void processPETSession(InveonSessionFiles inveonSessionFiles, String projectId) throws ServerException, IOException {
        ...
        while (iterator.hasNext()) {
            ...
            log.info("Header file: {}", headerFileName);
            log.info("Pixel file:  {}", pixelFileName);
            //factory.fillInveonHeaderMap(inveonImageRepresentation);
            ...
            log.debug("Scan Date: {}", scanDateString);
            log.debug("Scan Time: {}", scanTimeString);
            ...
            log.info("Subject ID: {}", subjectId.get());
            log.info("Session Label: {}", sessionLabel.get());
    ...
    private void transferFiles(Path temporarySessionFolder, Path scanFolder) throws IOException {
        ...
        try (DirectoryStream<Path> tempFileStream = Files.newDirectoryStream(temporarySessionFolder)) {
            for (Path tempFile: tempFileStream) {
                log.info("Source file in move: {}", tempFile.toString());
                Files.move(tempFile, scanFolder.resolve(tempFile.getFileName()));
            }
        }

        log.info("Delete temporary session folder: {}", temporarySessionFolder.toString());
...
//                recursiveFolderDelete(timestampDirectory.toFile());
    ...
    private void recursiveFolderDelete(File folder) {
        log.info("To Delete: {}", folder.getAbsolutePath());
    ...
    private void sendSessionBuildRequest(SessionData sessionData) {
        try {
            ...
            log.info("Sent PrearchiveOperationRequest {} {} {} {}", sessionData.getProject(), sessionData.getFolderName(), sessionData.getName(), sessionDir.getPath());
        } catch (Exception e) {
